...
def remap_speed(s):
    """
    get the speed in [0,200], readjust it so that if > 100 -> 150 and then anything
    over gets linearly fit between [150, 200] and if <100 ->50 and anything under
    gets linearly fit between [0,50]
    """
    if s == 100:
        return 100
    if s> 100:
        offset = s - 100
        scaled_offset = offset*(50/100)
        return int(150 + scaled_offset)
    else:
        offset = 100 - s
        scaled_offset = offset*(50/100)
        return int(100-scaled_offset)
...
class Mover:
    ...
    def set_motor_speed(self, left, right):
        ...
        self.ser.write(b'MTS' + bytes([left, right]))
...
class Robot:
    ...
    def __init__(self, hl, picoSer, id=1):
        self.hl = hl
    ...
    def track_once(self):
        ...
        offset = obj.x - 160  #320 pixel frame
        if abs(offset) < 30:
            offset = 0

        Kp_turn = 0.4
        turn_raw = offset*Kp_turn

        turn = clamp(turn_raw,-40,40)

        offset_norm = min(abs(offset)/160,1)
        forward_factor = 1-offset_norm*0.5
        base_speed = 160 * forward_factor
        ...
        left_speed_refit = clamp(int(left_speed),-100,100)+100
        right_speed_refit = clamp(int(right_speed),-100,100)+100

        left_speed_final = remap_speed(left_speed_refit)
        right_speed_final = remap_speed(right_speed_refit)


        self.mover.set_motor_speed(left_speed_final, right_speed_final)

        print(f"[Robot] Tracking: offset={offset}, height={obj.height}, "
              f"L={left_speed_final}, R={right_speed_final}, turn_factor={Kp_turn:.2f}")

        return "TRACKING"


# 1. Open serial connection to Pico & read commands
picoSer = serial.Serial("/dev/ttyAMA0", 115200, timeout=None)
print("Sending handshake...")
picoSer.write(b'PHL')
picoSer.read(3)
print("Handshake reply received! Reading instructions...")
mode = int.from_bytes(picoSer.read(1))
if mode == 1:
    algorithm = "ALGORITHM_TAG_RECOGNITION"
elif mode == 2:
    algorithm = "ALGORITHM_OBJECT_TRACKING"
else:
    print("Invalid mode, defaulting to tag recognition...")
    algorithm = "ALGORITHM_TAG_RECOGNITION"
